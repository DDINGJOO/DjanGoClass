...
class PokeDexImageSqlite3(PokeDexImageDataBaseInterface):
    ...
    def find_by_id(self, pokemon_id):
        with sqlite3.connect(self.DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM pokedex_images WHERE id=?", (pokemon_id,))
    ...
    def get_all_pokemons_images(self):
        with sqlite3.connect(self.DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM pokedex_images")
...
if __name__ == "__main__":
    ...
    print(poke_dex_image_repository.get_all_pokemons_images())
